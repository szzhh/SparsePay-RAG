#!/usr/bin/env bash
# =============================================================================
#  SparsePay-RAG: Natural Questions (NQ) experiment suite
#
#  Runs SparsePay-RAG across 4 backbone models × 4 privacy budgets (eps).
#  Supports: OPT-1.3B, Pythia-1.4B, Mistral-7B, Llama-3.1-8B
#
#  Usage:
#    bash run/run_nq.sh                    # run all models & budgets
#    bash run/run_nq.sh opt                # only OPT-1.3B
#    bash run/run_nq.sh opt 1              # OPT-1.3B with eps=1
# =============================================================================

set -euo pipefail

# --- Configuration ---
EVAL_SET="data/nq/nq-dev-small.questions"
GOLD_PATH="data/nq/nq-dev-small.qa.csv"
RETRIEVED_DIR="data/nq/retrieval_dpr"
PRED_DIR_BASE="output/nq"
DEVICE="cuda"
SEED=42
MAX_TOKENS=50
...
run_retrieval() {
    local eps="$1"
    echo "----------------------------------------------------------------------------"
    echo "  Retrieval (DP cluster-size noise) | NQ | eps=${eps}"
    echo "----------------------------------------------------------------------------"
...
run_one() {
    ...
    echo "----------------------------------------------------------------------------"
    echo "  SparsePay-RAG | NQ | ${model_name} | eps=${eps} | K=${n_split}"
    echo "----------------------------------------------------------------------------"
...
echo "All NQ experiments complete."